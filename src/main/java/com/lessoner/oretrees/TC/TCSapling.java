...
public class TCSapling extends BlockSapling {
...

	{
		...
		return metadata >= 0 && metadata < ItemTCSaplings.saplings.length ? this.iconLength[metadata] : this.iconLength[0];
	...
	public void func_149878_d(World w, int x, int y, int z, Random r) {
		...
		case 0:
			if (TreeOres.TCtrees) {
				object = new WorldGenTCTree(TreeOres.TCLog, TreeOres.TCLeaf, 0, 0, false, 4, 8, false);
			}
			break;
		case 1:
			if (TreeOres.TCtrees) {
				object = new WorldGenTCTree(TreeOres.TCLog, TreeOres.TCLeaf, 1, 1, false, 4, 8, false);
			}
			break;
		default:
			break;
