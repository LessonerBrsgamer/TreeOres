...
import com.lessoner.oretrees.worldgen.WorldGenBossTree;
import com.lessoner.oretrees.worldgen.WorldGenBossTree2;
...
public class BossSapling extends BlockSapling{
	...
	{ 
		...
		return metadata >= 0 && metadata <ItemBossSapling.saplings.length? this.iconLength[metadata] : this.iconLength [0]; 
    ...
    public void func_149878_d(World w, int x, int y, int z, Random r)
    {
        ...
        switch (l){
            case 0:
            	...
            	object = new WorldGenBossTree(TreeOres.blockLog, TreeOres.bossLeaf, 0,0,false, 15,20, false);
            	}
            	break;
            case 1: 
            	if(TreeOres.goldtree){
            	object = new WorldGenBossTree(TreeOres.blockLog, TreeOres.bossLeaf, 1,1,false, 15,20, false);
            	}
            	break;
            case 2:
            	if(TreeOres.coaltree){
            	object = new WorldGenBossTree(TreeOres.blockLog, TreeOres.bossLeaf, 2,2,false, 15,20, false);
            	}
            	break;
            case 3:
            	if(TreeOres.redstonetree){
            	object = new WorldGenBossTree(TreeOres.blockLog, TreeOres.bossLeaf, 3,3,false, 15,20, false);
            	}
            	break;
            case 4:
            	if(TreeOres.diamondtree){
            	object = new WorldGenBossTree2(TreeOres.blockLog2, TreeOres.bossLeaf2, 0,0,false, 15,20, false);            	
            	}
            	break;
            case 5:
            	if(TreeOres.emeraldtree){
            	object = new WorldGenBossTree2(TreeOres.blockLog2, TreeOres.bossLeaf2, 1,1,false, 15,20, false);            	
            	}
            	break;
            default:
                break;
